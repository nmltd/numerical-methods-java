...
/**
 * Runtime benchmark tests of symmetric positive definite matrix inverse.
 *
 * @author Song Lin
 */
public class MatrixInverseSPD implements BenchmarkOperation {

    private final int matrixSize;

    public MatrixInverseSPD(int matrixSize) {
        this.matrixSize = matrixSize;
    }

    @Override
    public String getTitle() {
        return String.format("symmetric matrix inverse (%dx%d)", matrixSize, matrixSize);
    }

    @Override
    public List<Argument> getArguments() {
        RandomNumberGenerator rng = new UniformRNG();
        Matrix A = new SymmetricMatrix(MatrixFactory.randomPositiveDefiniteMatrix(matrixSize, rng));
