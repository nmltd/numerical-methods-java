...
/**
 * Runtime benchmark tests of singular value decomposition (SVD) of random dense matrices.
 *
 * @author Ken Yiu
 */
public class SingularValueDecomposition implements BenchmarkOperation {

    private final int nRows;
    private final int nCols;

    public SingularValueDecomposition(int nRows, int nCols) {
        this.nRows = nRows;
        this.nCols = nCols;
    }
    ...
    public String getTitle() {
        return String.format("singular value decomposition (%dx%d)", nRows, nCols);
    }

    @Override
    public List<Argument> getArguments() {
        RandomNumberGenerator rng = new UniformRNG();
        Matrix A = MatrixFactory.randomDenseMatrix(nRows, nCols, rng);
